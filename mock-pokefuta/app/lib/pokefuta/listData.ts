...
export async function fetchRecentRows(
  ...
  return (recentData ?? [])
    .map((row) => {
      const ownershipRow = row as RecentOwnership;
      const pokefuta = Array.isArray(ownershipRow.pokefuta)
        ? ownershipRow.pokefuta[0] ?? null
        : ownershipRow.pokefuta ?? null;
      const user = Array.isArray(ownershipRow.users)
        ? ownershipRow.users[0] ?? null
        : ownershipRow.users ?? null;
      return {
        id: pokefuta?.id ?? 0,

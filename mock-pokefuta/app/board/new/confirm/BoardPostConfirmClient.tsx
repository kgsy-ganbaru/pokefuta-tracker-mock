...
export default function BoardPostConfirmClient() {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-6 pb-24">
      ...
      <section className="space-y-3 rounded-xl border bg-white p-4 shadow-sm"><div className="flex justify-between"><h2 className="font-semibold text-gray-800">出</h2><span className="text-sm text-gray-500">{draft.offers.length}種類</span></div>{renderSelections(draft.offers)}</section>
      <section className="space-y-3 rounded-xl border bg-white p-4 shadow-sm"><div className="flex justify-between"><h2 className="font-semibold text-gray-800">求</h2><span className="text-sm text-gray-500">{draft.wants.length}種類</span></div>{renderSelections(draft.wants)}</section>

...
export default function BoardPostConfirmClient() {
      ...
      {ids.map((id) => {
        ...
        return (
          <div key={id} className="rounded-xl border bg-white p-2 text-center shadow-sm">
            ...
            <p className="truncate text-[10px] text-gray-500">{row.pokemon_names}</p>
          </div>
        );
      })}
    </div>
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-6 pb-24">
      ...
      <section className="rounded-xl border bg-white p-4 shadow-sm"><p className="text-xs text-gray-500">投稿者</p><p className="mt-1 font-semibold text-gray-800">{draft.poster.nickname}</p></section>
      <section className="space-y-3 rounded-xl border bg-white p-4 shadow-sm"><div className="flex justify-between"><h2 className="font-semibold text-gray-800">出せるポケふた</h2><span className="text-sm text-gray-500">{draft.offers.length}種類</span></div>{renderSelections(draft.offers)}</section>
      <section className="space-y-3 rounded-xl border bg-white p-4 shadow-sm"><div className="flex justify-between"><h2 className="font-semibold text-gray-800">欲しいポケふた</h2><span className="text-sm text-gray-500">{draft.wants.length}種類</span></div>{renderSelections(draft.wants)}</section>
      ...
      {submitted && <p role="status" className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">投稿を受け付けました。現在はモックのため、データは保存されません。</p>}
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-center"><button type="button" onClick={() => router.push("/board/new")} className="rounded-full border bg-white px-6 py-3 text-sm font-semibold text-gray-700">入力内容を修正する</button><button type="button" onClick={() => setSubmitted(true)} className="rounded-full bg-emerald-500 px-6 py-3 text-sm font-semibold text-white hover:bg-emerald-600">この内容で投稿する</button></div>
    </main>
  );
}

...
export default function BoardCommentsMock({ initialComments }: { initialComments: BoardComment[] }) {
  ...
  return (
    <section className="space-y-4 rounded-xl border bg-white p-4 shadow-sm">
      ...
      {comments.length === 0 ? <p className="rounded-lg bg-gray-50 px-4 py-6 text-center text-sm text-gray-500">まだコメントはありません。</p> : <div className="divide-y">{comments.map((comment) => <article key={comment.id} className="py-4 first:pt-0 last:pb-0"><div className="flex items-center justify-between gap-3"><Link href={`/users/${comment.user.id}`} className="text-sm font-semibold text-emerald-700 hover:underline">{comment.user.nickname}</Link><time className="shrink-0 text-xs text-gray-400">{comment.postedAt}</time></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-gray-700">{comment.body}</p></article>)}</div>}

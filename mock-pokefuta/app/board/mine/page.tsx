...
export default async function MyBoardPostsPage({ searchParams }: { searchParams: Promise<{ from?: string }> }) {
  const { from } = await searchParams; const fromAccount = from === "account"; const supabase = await createClient(); const user = await getAuthProfile(supabase);

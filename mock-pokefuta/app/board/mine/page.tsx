...
export default async function MyBoardPostsPage({ searchParams }: { searchParams: Promise<{ from?: string }> }) {
  ...
  return <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6"><header>{!fromAccount && <Link href="/board" className="text-sm font-semibold text-emerald-700">← 掲示板に戻る</Link>}<h1 className={`${fromAccount ? "" : "mt-3"} text-xl font-semibold text-gray-800`}>自分の投稿</h1><p className="mt-2 text-sm text-gray-500">募集中と終了済みの投稿をまとめて確認できます。</p></header>{supabase && !user ? <section className="rounded-xl border bg-white p-6 text-center"><p className="text-sm text-gray-600">自分の投稿を見るにはログインしてください。</p><Link href="/account" className="mt-4 inline-block font-semibold text-emerald-700">ログインへ</Link></section> : mine.length === 0 ? <section className="rounded-xl border border-dashed bg-white p-8 text-center"><p className="text-sm text-gray-600">投稿はまだありません。</p><Link href="/board/new" className="mt-4 inline-block font-semibold text-emerald-700">新しく投稿する</Link></section> : <div className="space-y-4">{mine.map((thread) => { const active = isThreadActive(thread); return <article key={thread.id} className="space-y-4 rounded-2xl border bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><Link href={`/board/${thread.id}`} className="font-semibold text-emerald-700 hover:underline">出せる {thread.offers.length}種類 ／ 欲しい {thread.wants.length}種類</Link><p className="mt-1 text-xs text-gray-500">更新 {thread.updatedAt}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${active ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-600"}`}>{active ? `あと${remainingDays(thread)}日` : "募集終了"}</span></div><p className="line-clamp-2 text-sm text-gray-600">{thread.comment || "コメントはありません。"}</p>{supabase ? <p className="text-xs text-gray-500">編集・募集管理は次の実装で接続します。</p> : <ThreadManagementMock initiallyActive={active} threadId={thread.id} />}</article>; })}</div>}</main>;
}
